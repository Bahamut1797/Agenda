<template>
    <div>
            ...
            <div class="modal-dialog" role="document" >
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">Reschedule reminder</h4>
                    </div>
                    <div class="modal-body">
                        <span>Do you really want to reschedule this reminder?</span>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
                        <span class="pull-right">
                            <button type="button" class="btn btn-primary" v-on:click="reschedule()">
                                Continue
                            </button>
                        </span>
                    </div>
        ...
        <div class="panel panel-default">
            <div class="panel-heading">View reminder</div>
            <div class="panel-body">
                <form v-on:submit.prevent="saveForm()">
                    <div class="row">
                        <div class="col-xs-7 form-group">
                            <label id="myElement" class="control-label">Reminder title</label>
                            <input v-bind:disabled="newReminder == false" type="text" required v-model="reminder.title" class="form-control">
                        </div>
                        <div class="col-xs-5 form-group">
                            <label class="control-label" style="display:block;" >Category</label>
                            <select v-bind:disabled="newReminder == false" v-on:change="handleChange" v-model="reminder.category" class="form-control" style="width:70%; display:unset;" >
                                <option v-for="category in categories" v-bind:value="category.id" :key="category.id">
                                    {{ category.name }}
                                </option>
                            </select>
                            <button v-if="newReminder == false" v-bind:disabled="newReminder == false" class="btn btn-default" style="border:none; padding:6px 4px;" v-on:click.prevent=";" data-toggle="modal" data-target="#myModal" >
                                <img src="/open-iconic/svg/plus.svg" width="15" alt="add category" />
                            </button>
                            <button v-bind:disabled="newReminder == false" v-if="categoryName != 'Default' && newReminder == false" class="btn btn-default" style="border:none; padding:6px 4px;" v-on:click.prevent=";" data-toggle="modal" data-target="#myModalDelete" >
                    ...
                    <div class="row">
                        <div class="col-xs-12 form-group">
                            ...
                            <textarea v-bind:disabled="newReminder == false" id="description" class="form-control" maxlength='1000' rows='4' placeholder="A brief description" v-model="reminder.description"></textarea>
                    ...
                    <div class="row">
                        <div class="col-xs-4 form-group">
                            ...
                            <input v-bind:disabled="newReminder == false" type="checkbox" v-model="reminder.isPayment">
                        </div>
                        <div v-if="reminder.isPayment == 1" class="col-xs-8 form-group">
                            <label class="control-label">Amount</label>
                            <input v-bind:disabled="newReminder == false" type="number" v-bind:required="reminder.isPayment == 1 ? true : false"  min="0" step=".01" v-model="reminder.amount" class="form-control">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 form-group">
                            <label class="control-label">Location</label>
                            <gmap-autocomplete v-bind:disabled="newReminder == false" id="map" class="form-control"></gmap-autocomplete>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-6 form-group">
                            <label class="control-label">Contact Number</label>
                            <input v-bind:disabled="newReminder == false" type="tel" v-model="reminder.contact" class="form-control" placeholder="+525512345678">
                        </div>
                        <div class="col-xs-6 form-group">
                            <label class="control-label">CC (Email)</label>
                            <input v-bind:disabled="newReminder == false" type="email" v-model="reminder.secEmail" class="form-control" placeholder="example@domain.com">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-6 form-group">
                            <label class="control-label">Alarm Date</label>
                            <input v-bind:disabled="newReminder == false" id="date" type="date" v-model="reminder.alarmDate" class="form-control">
                        </div>
                        <div class="col-xs-6 form-group">
                            <label class="control-label">Alarm Time</label>
                            <input v-bind:disabled="newReminder == false" type="time" required v-model="reminder.alarmTime" class="form-control">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-6 form-group">
                            <label class="control-label">Frecuency</label>
                            <select v-bind:disabled="newReminder == false" v-model="reminder.frecuency" class="form-control">
                    ...
                    <div class="row">
                        <div v-if="newReminder == true" class="col-xs-12 form-group">
                    ...
                    <div class="row">
                        <div class="col-xs-12 form-group">
                            <button v-if="newReminder == false" type="button" v-on:click.prevent=";" class="btn btn-success" data-toggle="modal" data-target="#myModalReschedule">Reschedule</button>
                            <button v-if="newReminder == true" type="submit" class="btn btn-success">Create</button>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    export default {
        mounted() {
            ...
            app.reminderId = id;
            axios.get('/api/v1/archive_reminders/' + id)
                .then(function (resp) {
        ...
        methods: {
            saveForm() {
                ...
                axios.post('/api/v1/reminders/', newReminder)
                    .then(function (resp) {
                        $.notify("Reminder reschedule", "success");
                        app.$router.replace('/');
                    })
                    .catch(function (resp) {
                        console.log(resp);
                        $.notify("Could not reschedule your reminder", "error");
                    });
            },
            saveCategory() {
                var app = this;
            ...
            reschedule() {
                ...
                var app = this;
                app.newReminder = true;
                app.reminder.alarmDate = currentDate;
                app.reminder.archiveIt = true;
                app.reminder.isSecret = false;

                $( "#date" ).attr( "min", currentDate );
                $('#myModalReschedule').modal('hide');
            }
        }
    }
</script>
